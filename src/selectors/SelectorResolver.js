...
class SelectorResolver {
  ...
  resolve(selectorConfig) {
    ...
    if (typeof selectorConfig === 'object' && selectorConfig !== null) {
      ...
      if (!by) {
        throw new Error(
          'Selector object must have "by" property. ' + 'Example: { by: "id", value: "username" }'
        );
      }
      ...
      Logger.debug(`Resolved selector: ${by}="${value}" -> "${resolvedSelector}"`);
  ...
  isObjectFormat(selectorConfig) {
    return typeof selectorConfig === 'object' && selectorConfig !== null && 'by' in selectorConfig;
  }
}
