...
import AssertionExecutor from '../assertions/AssertionExecutor.js';
...
class TestExecutor {
  ...
  async executeTestInternal(testData, testReport) {
    ...
    // Initialize enhanced executors (supporting new YAML format)
    const actionExecutor = new ActionExecutor(this.page, screenshotManager);
    const assertionExecutor = new AssertionExecutor(this.page, screenshotManager);
    ...
    try {
      ...
      // Execute fill data phase
      if (testData.fillData) {
        Logger.info('\n--- FILL DATA PHASE ---');
        for (const action of testData.fillData) {
          let step;
          
          // Check if using new object-based selector format
          const isNewFormat = typeof action.selector === 'object' || action.capture !== undefined;
          
          if (isNewFormat) {
            // Use enhanced ActionExecutor for new format
            step = await actionExecutor.execute(action, testData.name, stepIndex++, 'FILL');
          } else {
            // Use legacy ActionHandler for backward compatibility
      ...
      if (testData.assertions) {
        ...
        for (const assertion of testData.assertions) {
          // Check if using new assertion format with enhanced features
          const isNewFormat = assertion.operator || 
                             assertion.type === 'textContent' || 
                             assertion.type === 'inputValue' ||
                             assertion.type === 'visibility' ||
                             assertion.type === 'enabled' ||
                             assertion.type === 'style';
          
          if (isNewFormat) {
            // Use enhanced AssertionExecutor
            const steps = await assertionExecutor.execute(assertion, testData.name, stepIndex);
            ...
            // Use legacy AssertionEngine for backward compatibility
            let step;
